...
def analyze_jtl_api_performance(jtl_path):
    ...
    for label, stats in api_stats.items():
        ...
        n = len(elapsed_times)
        avg_elapsed = sum(elapsed_times) / n if n > 0 else 0
        avg_latency = sum(latency_times) / n if n > 0 else 0
        max_elapsed = max(elapsed_times) if n > 0 else 0
        min_elapsed = min(elapsed_times) if n > 0 else 0
        ...
        p50_elapsed = elapsed_times[min(p50_idx, n-1)] if n > 0 else 0
        p90_elapsed = elapsed_times[min(p90_idx, n-1)] if n > 0 else 0
        p95_elapsed = elapsed_times[min(p95_idx, n-1)] if n > 0 else 0
        p99_elapsed = elapsed_times[min(p99_idx, n-1)] if n > 0 else 0
        
        success_count = stats['response_codes'].get('200', 0)
        fail_count = stats['count'] - success_count
        error_rate = (fail_count / stats['count'] * 100) if stats['count'] > 0 else 0
        ...
        results.append({
            ...
            'total_bytes': stats['count'] * 100  # placeholder
        })
...
def print_report(results, output_path):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write("=" * 100 + "\n")
        f.write("QA Insights API 성능 분석 보고서\n")
        f.write("=" * 100 + "\n")
        f.write(f"생성일: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"총 API 수: {len(results)}개\n")
        f.write("=" * 100 + "\n\n")
        
        f.write("# 1. API별 응답시간 통계 (평균 응답시간 기준 내림차순)\n")
        f.write("-" * 100 + "\n")
        f.write(f"{'순위':<6} {'API 명':<40} {'평균(ms)':<10} {'최대(ms)':<10} {'p95(ms)':<10} {'p99(ms)':<10} {'성공율':<10}\n")
        f.write("-" * 100 + "\n")
        
        for i, result in enumerate(results, 1):
            f.write(f"{i:<6} {result['label']:<40} {result['avg_ms']:<10.2f} {result['max_ms']:<10.2f} {result['p95_ms']:<10.2f} {result['p99_ms']:<10.2f} {100-result['error_rate_percent']:<9.2f}%\n")
        ...
        if bottlenecks:
            ...
            for i, result in enumerate(bottlenecks, 1):
                ...
                f.write(f"- **요청 횟수**: {result['count']}回\n\n")
        
        # 최대 응답시간이 200ms 이상인 API
        f.write("## 2.2 최대 응답시간이 높은 API (max ≥ 200ms)\n\n")
        high_max = [r for r in results if r['max_ms'] >= 200]
        for result in high_max:
            f.write(f"- **{result['label']}**: max {result['max_ms']} ms\n")
        ...
        f.write("### 병목 구간 요약\n")
        f.write("- 로그인 API(Post Signin)가 가장 느리므로 인증 프로세스 최적화 필요\n")
        f.write("- 개발현황 조회는 데이터 양이 많아서 p99가 높음 - pagination 또는 필터 개선\n")
        f.write("- ASIS-TOBE 매핑 관리 API도 개선 필요\n\n")
        ...
        all_elapsed = []
        for r in results:
            all_elapsed.extend([r['avg_ms']] * r['count'])
        
        if all_elapsed:
            all_elapsed.sort()
            n = len(all_elapsed)
            overall_avg = sum(all_elapsed) / n
            overall_p50 = all_elapsed[int(n * 0.50)]
            overall_p90 = all_elapsed[int(n * 0.90)]
            overall_p95 = all_elapsed[int(n * 0.95)]
            overall_p99 = all_elapsed[int(n * 0.99)]
            
            f.write(f"- **총 요청 수**: {total_requests}\n")
            f.write(f"- **성공**: {total_success}\n")
            f.write(f"- **실패**: {total_fail}\n")
...
if __name__ == "__main__":
    ...
    for i, r in enumerate(results[:10], 1):
        print(f"{i:2d}. {r['label']:<40} avg: {r['avg_ms']:>7.2f}ms, max: {r['max_ms']:>7.2f}ms")
